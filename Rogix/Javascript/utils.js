	...
	class Connection{
		...
		connect(address, callback){
			...
			doGet(address + '/con/connect', {}, (res)=>{
				if(res == null) console.log("Connection failed, couldn't connect.")
				else{
					console.log("Connected with ID: " + res)
		...
		tick(callback){
			doGet(this.address + '/con/request', {connectionID:this.connectionID}, (res)=>{
				if(res.data != null){
					callback(res.data)
				}
			})
		...
		/* It is event based
		let c = new Connection(website, function(data){
			alert(data)
		})
		*/
